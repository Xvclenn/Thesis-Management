...
function formatMongolianTitle(year: number, month: number) {
    return `${year} оны ${MONTHS[month]}`;
}
...
const EventDetailModal: React.FC<EventDetailModalProps> = ({
    ...
    return (
        <div
            className="flex items-center justify-center fixed inset-0 isolate z-50 bg-black/10 duration-100 supports-backdrop-filter:backdrop-blur-xs"
            onClick={onClose}
        >
            <div
                className="bg-white border-gray-200 rounded-xl p-6 w-75 shadow-2xl mx-4"
                onClick={(e) => e.stopPropagation()}
            >
                <div
                    className="h-2.5 -mx-6 -mt-6 mb-4 rounded-t-xl"
...
const CustomToolbar: React.FC<CustomToolbarProps> = ({
    ...
    <div className="flex flex-wrap items-center justify-between gap-2 mb-3 px-1">
        ...
        <div className="flex items-center gap-1.5">
            <button
                onClick={onPrev}
                className="p-1.5 rounded-lg border border-gray-200 text-gray-500 hover:bg-orange-50 hover:border-orange-300 hover:text-orange-500 transition-colors cursor-pointer"
            >
                <ChevronLeft size={16} />
            </button>
            <button
            ...
            <button
                onClick={onToday}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-orange-400 bg-orange-500 text-white text-xs font-medium hover:bg-orange-600 transition-colors cursor-pointer"
            >
        ...
        {/* Center: Mongolian title */}
        <div className="text-sm font-semibold text-gray-800 order-first w-full sm:order-0 sm:w-auto text-center">
            {formatMongolianTitle(year, month)}
        </div>

        {/* Right: view switcher */}
        <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-0.5">
            {(["dayGridMonth", "timeGridWeek", "timeGridDay"] as CalView[]).map(
                (v) => (
                    <button
                        key={v}
...
const StudentCalendar: React.FC = () => {
    ...
    return (
        <div className="flex flex-col gap-4 py-3 relative font-inherit">
            ...
            <div className="flex flex-col lg:flex-row gap-4 relative min-h-[600px]">
                {/* ── Main Calendar ── */}
                <div className="flex-1 min-w-0">
                    <div className="p-3 border border-gray-200 rounded-xl overflow-hidden bg-white">
                        {/* Custom Toolbar */}
                        <CustomToolbar
                            year={year}
                            month={month}
                        ...
                        {/* FullCalendar — headerToolbar hidden, controlled via CustomToolbar */}
                        <FullCalendar
                            ref={calRef}
                            plugins={[
                                dayGridPlugin,
                ...
                {/* ── Desktop Sidebar ── */}
                <div className="hidden lg:flex w-57.5 shrink-0 flex-col gap-3">
                    <MiniCalendar
                        year={year}
                        month={month}
                        today={today}
